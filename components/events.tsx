  ...
  return (
    <section id="events" className="px-4 py-24">
      <SectionHeading
        ...
        description="From the first swirl of henna to the final toast, here is everything you need to know, including what to wear for each evening."
      />
...
function EventCard({ event, index }: { event: WeddingEvent; index: number }) {
  ...
  return (
    <article
      className={`group relative flex flex-col overflow-hidden rounded-3xl border border-white/70 bg-white/80 p-6 shadow-[0_24px_50px_-28px_rgb(90_74_94_/_0.45)] backdrop-blur transition-transform duration-500 hover:-translate-y-1 ${index === 4 ? "sm:col-span-2 lg:col-span-1" : ""}`}
    >
      <div
      ...
      <div className="relative">
        ...
        <h3 className="font-script mt-2 text-5xl leading-none text-plum">{event.name}</h3>
